...
#### t_receivings（受入データ）
id(PK), order_id(FK), received_date, received_qty, warehouse_id(FK), remarks, user_id, created_at, updated_at

#### t_dispatches（払い出しデータ）
id(PK), dispatch_date, item_id(FK), dispatch_qty, warehouse_id(FK), destination, department_id(FK), remarks, user_id, created_at, updated_at
