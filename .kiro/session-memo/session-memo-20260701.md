...
## チェックポイント（コピー廃止の残骸除去・単一正本 確定）

### 実施
- `.kiro/skiles`→`.kiro/skills`（git mv）。
- project-rules を単一正本ルールへ改定（2箇所配置廃止・開始時参照修正）。
- コピー削除: MaterialModule/docs/specs（48件 git rm）・CommonModule/docs/specs。print-platform 正本の「2箇所配置」記述・CommonModule/docs/README を単一正本表記へ更新。
- **残骸除去**: `git rm` はファイルのみ削除し空ディレクトリを残すため、MaterialModule/docs/specs に空フォルダ（delivery-monitor-page・order-recommendation）が残存 → 物理削除。CommonModule 側は残骸なしを確認。
- 空フォルダは git 未追跡のため追加コミット不要（両 repo status=0）。

### コミット
- MaterialModule `1fe3526` / CommonModule `d323664` / Nonaka `5b81b47`。

### 最終状態
- spec は `.kiro/specs/{Module}/{feature}/` 単一正本（CommonModule=2 / MaterialModule=19）。モジュール別コピー全廃・ドリフト解消。
- MaterialModule/docs=資材固有のみ、CommonModule/docs=sql/README/smtp手順のみ。clnCoCore 無変更。

### 次
- ドキュメント/spec/steering 整理は完了。print-platform 実装フェーズ（Wave0: 1.1 DDL＝CommonModule/docs/sql に t_print_queue・m_print_agent_control）へ着手可能。

---

## print-platform 実装：タスク1.1 完了（Wave0）

- サブエージェント委譲で **`CommonModule/docs/sql/` に DDL 2ファイル新設**:
  - `create_t_print_queue.sql`（16列・FAX無・row_version・CK print_status∈{0,1,2,3,9}・index 3種）
  - `create_m_print_agent_control.sql`（1行運用・row_version無・初期1行INSERT）
  - 既存 `create_print_agent_control.sql`(db_material_dev版)は別物として保持。
- SQL作成のみ（DDL適用・ビルド・テストはユーザー側）。診断クリア。tasks.md 1.1 を completed。
- **ユーザー作業（任意）**: db_common_dev に上記2 SQL を実行。

### 次 ready（Wave1・3件）: 1.2 テーブル定義書/ER図更新・2.1 TPrintQueue・2.2 MPrintAgentControl。1単位ごと確認方針で、ユーザー承認後に着手。

---

## print-platform 実装：タスク1・2 完了（CommonModule 基盤）

### 完了（最小単位・1件ずつ委譲）
- 1.1 DDL（create_t_print_queue.sql / create_m_print_agent_control.sql）＝`CommonModule/docs/sql/`
- 1.2 テーブル定義書・ER図に print-platform 2テーブル追記（`.kiro/docs/db/`）
- 2.1 `CommonModule/Data/Entities/TPrintQueue.cs`（16列・row_version・FAX無）
- 2.2 `CommonModule/Data/Entities/MPrintAgentControl.cs`（1行運用・row_version無）
- 2.3 `CommonModule/Data/CommonDbContext.cs` に DbSet 2件追加（PrintQueue / PrintAgentControls）
- 親タスク1・2 完了。全て診断クリア。ビルド/テスト/DDL適用はユーザー側。

### 進捗: 6/37 完了・31残。次 ready（Wave3・3件）
- 3.1 `IPrintQueueService`/`PrintQueueService` 実装（投入サービス）
- 4.1 Common_PrintMonitor PageModel（一覧・フィルタ・サマリ）
- 4.4 死活判定
- 最小単位方針で、ユーザー確認後に着手。

### 留意
- IDE の「i.map is not a function」等は Kiro UI 一時エラー（Restart で回復・コード/spec無関係）。並行より1件ずつが安全。
- 未コミット: CommonModule（entities/DbContext/sql）・Nonaka/.kiro（specs 進捗・docs/db 更新）。区切りでコミット推奨。

---

## チェックポイント（print-platform 実装中・コンテキスト逼迫ハンドオフ用）

### 現在地
- spec: `.kiro/specs/CommonModule/print-platform/`（単一正本・入れ子）。tasks 進捗 **10/37 完了**。
- 完了タスク: 1（DDL＋Doc）/ 2（TPrintQueue・MPrintAgentControl・DbContext DbSet）/ 3（IPrintQueueService・PrintQueueService＋DI）/ 4.1（PrintMonitor PageModel 一覧・フィルタ・サマリ）/ 4.4（死活判定）。
- 作成/変更済みファイル（CommonModule）:
  - `Data/Entities/TPrintQueue.cs`・`Data/Entities/MPrintAgentControl.cs`
  - `Data/CommonDbContext.cs`（DbSet: PrintQueue / PrintAgentControls 追加）
  - `Services/IPrintQueueService.cs`・`Services/PrintQueueService.cs`
  - `Extensions/CommonModuleExtensions.cs`（AddScoped<IPrintQueueService>）
  - `Areas/Common/Pages/PrintMonitor/Index.cshtml.cs`（4.1＋4.4。ビュー未作成）
  - `docs/sql/create_t_print_queue.sql`・`create_m_print_agent_control.sql`
- `.kiro/docs/db/テーブル定義書.md`・`ER図.md` に print-platform 2テーブル追記済み。
- コミット済み: CommonModule（`40feede` ほか）/ Nonaka（`51183d3` ほか）。**4.1・4.4 は未コミット**。

### 次に行う1アクション
- **タスク 4.6「再出力 `OnPostReprintAsync` を実装」**（`PrintMonitor/Index.cshtml.cs` に追記）。
  - 完了(3)・エラー(9) かつ payload または pdf_path が非空のジョブのみ print_status=1 に戻し picked_at/printed_at/error_message クリア・updated_at=UtcNow。待機/処理中/対象外は拒否通知。ソース無しは通知。DbUpdateConcurrencyException 捕捉で競合メッセージ。
  - 参照作法: `SmtpMonitor/Index.cshtml.cs` の `OnPostResendAsync`。
  - _Requirements: 2.2, 2.3, 9.4, 9.5_

### 監視画面の残り
- 4.6 再出力 → 4.8 ビュー `Index.cshtml`（`Common_SmtpMonitor` の Index.cshtml 準拠）。任意テスト 4.2/4.3/4.5/4.7（`*`・CommonModule.Tests）。

### 以降（Wave）
- 6.x PrintAgent（別ソリューション: `\\OJIADM23120073\Labs\WindowsService\PrintAgent`）エンティティ・DbContext・接続先 db_common_dev。
- 7.x Worker デュアルモード・状態遷移・row_version・並行統合。8.CP。9.カットオーバー移行SQL＋Spec同期。10.最終CP。

### 運用ルール（厳守）
- 1ターン=1タスクで完全に区切る（UIエラー `i.map`/`undefined` 回避）。UIエラーは Kiro スペックパネルのUI一時エラー＝Restart で回復・コード/spec 無傷。
- MainWeb・SharedCore・AuthModule は変更不可。成果物は CommonModule 内。ビルド・テスト・DDL適用・実印刷はユーザー側。
- Spec 単一正本（`.kiro/specs/{Module}/{feature}/`・コピー無し）。`.kiro` は Nonaka ルート git（.gitignore で .kiro のみ追跡）で版管理。
- task_update ツールが稀に「not available」になる → tasks.md のチェックボックスが正であることを確認して継続。

### 再開合図
「再開します、session-memoを確認」。最新は本ファイル（20260701）。

---

## print-platform 実装：タスク4.6 完了（再出力 OnPostReprintAsync）

### 完了（最小単位・サブエージェント委譲）
- **4.6 再出力 `OnPostReprintAsync` を実装**（`CommonModule/Areas/Common/Pages/PrintMonitor/Index.cshtml.cs` に追記・4.1/4.4 は不変）。
  - 完了(3)・エラー(9) かつ（`print_payload` または `pdf_path` の一方が非空）のみ `print_status=1` に戻し `picked_at`/`printed_at`/`error_message` クリア・`updated_at=UtcNow`。
  - 待機(1)・処理中(2)・対象外(0) は不正遷移として拒否通知。payload・pdf_path 双方無しは「印刷ソースが無い」旨を通知。
  - ジョブは追跡ロード（AsNoTracking無し）で row_version 楽観ロック実効化。`DbUpdateConcurrencyException` 捕捉で「他のユーザーが先に更新しました。画面を再読み込みしてください。」
  - `SmtpMonitor/OnPostResendAsync` の作法に準拠（TempData通知・RedirectToPage・日本語コメント・StatusText 再利用）。`using Microsoft.AspNetCore.Mvc;` 追加。
  - 診断クリア。ビルド/テストはユーザー側。MainWeb/SharedCore/AuthModule 不変更。
- tasks 進捗 **12/37 完了**（4.6 completed）。未コミット（4.6 差分）。

### 実ファイルパス注記
- サブエージェント報告: PrintMonitor PageModel の実体パスは `Nonaka\CommonModule\Areas\Common\Pages\PrintMonitor\Index.cshtml.cs`（`clnCoCore` セグメント無し）。

### 次に行う1アクション
- **タスク 4.8「監視画面ビュー `Index.cshtml` を実装」**（Area "Common" 共通スタイル・Bootstrap5＋vanilla JS・site.css 不変更。一覧・フィルタ・サマリ・死活表示・再出力操作・error_message 表示。`Common_SmtpMonitor` の Index.cshtml 準拠）。_Requirements: 9.1, 9.6, 9.7, 10.1_
- 残りの wave4 テスト（3.2/3.3/4.2/4.3/4.5・`*`任意）は省略可。4.8 は非任意で監視画面の最終ピース。
- 以降: 5.CP → 6.x PrintAgent（別sln）→ 7.x Worker → 8.CP → 9 カットオーバー → 10 最終CP。
...
## print-platform 実装：タスク4.8 完了（監視画面ビュー）＋親タスク4 完了

### 完了（最小単位・サブエージェント委譲）
- **4.8 監視画面ビュー `Index.cshtml`** 新規作成（`CommonModule/Areas/Common/Pages/PrintMonitor/Index.cshtml`・既存 PageModel にバインド・.cs 不変更）。
  - ヘッダに PrintAgent 死活表示（ポーリング中/応答なし・最終応答JST・マシン名, 要件9.6）／成功・エラーアラート／サマリカード（待機・処理中・完了・エラー）／GETフィルタ（Status/ReportType/Keyword=参照コード/DateFrom/To）。
  - 一覧（ID降順）: ID・モジュール・レポート種別・参照コード・ステータスbadge(0/1/2/3/9)・部数・PDF有無・作成/取得/印刷/更新日時(UTC→JST)・エラー内容(tooltip・要件9.7)。
  - 再出力: PrintStatus 3/9 の行に POST（asp-page-handler="Reprint"＋hidden id）。サーバ側で payload/pdf_path 条件を強制。SmtpMonitor の再送作法に準拠。
  - ページャ（前へ/次へ・ページサイズ 10/20/30/50/100・フィルタ保持）／Bootstrap5＋vanilla JS（tooltip・10秒自動更新 sessionStorage）。MainWeb/site.css 不変更（要件10.1）。診断クリア。
- **CommonModule.sln ビルド完了（ユーザー確認済み・4.6分）**。4.8 は同モジュール・ビルド確認は次回まとめて可。
- 親タスク **4「共通監視画面 Common_PrintMonitor」完了**（子の残りは任意PBT 4.2/4.3/4.5/4.7 のみ）。
- tasks 進捗 **14/37 完了**。未コミット（4.6 .cs＋4.8 .cshtml）。

### 次に行う1アクション
- タスク **5「チェックポイント - CommonModule のテストを通す」**（CommonModule.Tests。任意PBT 3.2/3.3/4.2/4.3/4.5 を書くならここ。テスト実行はユーザー側）。
- その後 wave6: **6.1/6.2/6.3 PrintAgent（別ソリューション `\\OJIADM23120073\Labs\WindowsService\PrintAgent`）** エンティティ・DbContext・接続先 db_common_dev。→ 7.x Worker → 8.CP → 9 カットオーバー → 10 最終CP。
...
## print-platform: 監視画面の認可まわり 結論（ユーザー確定）

### 確認事項
- CommonModule は **SharedCore と同格の「他プロジェクトから利用される共有基盤」**。`CommonModule.csproj` は SharedCore を ProjectReference（`..\clnCoCore\SharedCore\SharedCore.csproj`）＋ EF Core（`Microsoft.EntityFrameworkCore`/`.SqlServer` 8.0.*）で `CommonDbContext`(db_common_dev)。他モジュールは `IPrintQueueService`/`ISmtpQueueService` 契約経由で利用。構築基準（steering: module-development-guide/structure/coding-standards に取り込み済み）に準拠。
- 監視画面（/Common/PrintMonitor・/Common/SmtpMonitor）は **MaterialModule と同じく MainWeb にホストされる**ため、`[Authorize(Policy="DbPermissionCheck")]` がそのまま有効。
- **モニタリングページのアクセス権限（認証DB dbAuthTest の m_content / r_content_auth 登録）は運用側（ユーザー）で設定**する。

### 確定・実施
- smtp-sender 実装では SmtpMonitor に対し `[Authorize]` 付与のみで、Common エリアの content 権限登録SQLは存在しない（SuperUser 運用想定）。「SmtpMonitor と同じ手順」という確立手順は無い。
- 今セッションで一旦作成した `CommonModule/docs/sql/register_common_print_monitor_content.sql` は **不要のため削除**（権限登録は運用側・前例なし・ドリフト防止）。
- 結論: PrintMonitor 側のコード認可は `[Authorize(Policy="DbPermissionCheck")]` 済みで完了。権限データ登録はユーザー作業。

### print-platform 進捗
- 14/37 完了（1・2・3・4 完了）。未コミット: 4.6 `Index.cshtml.cs`＋4.8 `Index.cshtml`。
- 次: タスク5（CP: CommonModule.Tests 実行＝ユーザー側）／任意PBT（3.2/3.3/4.2/4.3/4.5/4.7）→ wave6 **6.1/6.2/6.3 PrintAgent（別sln `\\OJIADM23120073\Labs\WindowsService\PrintAgent`）** エンティティ・DbContext・接続先 db_common_dev。

---

## print-platform 実装：タスク6.1 完了（PrintAgent 側 TPrintQueue）＋重要な認識訂正

### 重要訂正: PrintAgent/SmtpAgent は編集可能範囲内
- system 設定の編集可能フォルダは **2つ**: `...\CoCore\Nonaka` と `...\Labs\WindowsService`。
- 先に「PrintAgent はワークスペース外で編集不可」と誤判定したのは、**パスを大文字 `OJIADM` で指定したため**。**小文字 `ojiadm23120073` なら WindowsService 配下（PrintAgent/SmtpAgent）も編集可能**。以後パスは小文字で扱う。
- PrintAgent・SmtpAgent は Web ではなく独立 Worker（BackgroundService・別sln・別git）だが、本ワークスペースから直接改修可能。

### 完了（6.1・サブエージェント委譲）
- `\\ojiadm23120073\Labs\WindowsService\PrintAgent\Models\TPrintQueue.cs` 新規作成（namespace PrintAgent.Models・`[Table("t_print_queue")]`）。CommonModule 側 `TPrintQueue` と同一列・同一型（id/module/report_type/reference_code/output_type/print_status/print_payload/pdf_path/printer_name/copies/picked_at/printed_at/error_message/created_at/updated_at/row_version[Timestamp]）。診断クリア。
- `TOrderReport` からの差分: module・pdf_path・row_version 追加／fax_status 削除／completed_at・print_at → printed_at 一本化。
- `TOrderReport.cs` は 6.2（DbContext差替え）まで残置（今削除するとビルド不整合）。

### 進捗・次
- tasks 15/37 完了（6.1 = `[x]`）。`task_update` ツールは本セッション使用不可 → tasks.md チェックボックスを正として運用。
- **次の1アクション: 6.2 `PrintAgentDbContext` 改修**（`DbSet<TOrderReport> OrderReports` → `DbSet<TPrintQueue> PrintQueue`・`ToTable("t_print_queue")`。MPrintAgentControl は継続）。
- ⚠ **6.2 と 7.1 はビルド上カップリング**: 6.2 で DbSet を差し替えると `PrintJobWorker`（`db.OrderReports` 参照）がコンパイル不整合になる。7.1（Worker のデュアルモード・新エンティティ対応）まで進めて初めてビルド可能。→ 6.2 → 6.3 → 7.1 をまとめて実施し、その後ユーザービルドが安全。
- 未コミット: PrintAgent（TPrintQueue.cs 追加）・Nonaka/.kiro（tasks 進捗・本メモ）。

---

## print-platform 実装：タスク6.2 完了（PrintAgentDbContext 差替え）

- `\\ojiadm23120073\Labs\WindowsService\PrintAgent\Data\PrintAgentDbContext.cs`:
  - `DbSet<TOrderReport> OrderReports` → `DbSet<TPrintQueue> PrintQueue`
  - `OnModelCreating`: `Entity<TOrderReport>().ToTable("t_order_reports")` → `Entity<TPrintQueue>().ToTable("t_print_queue")`
  - `MPrintAgentControl`（`m_print_agent_control`）は継続。
  - DbContext 単体は診断クリア。
- ⚠ 中間状態: `PrintJobWorker` が `db.OrderReports` 参照のため **7.1 まで PrintAgent はビルド不可**（想定どおり・ユーザー選択で1つずつ進行）。`TOrderReport.cs` は残置（7.1後に不要化・別途整理可）。
- tasks 16/37 完了（6.2 = `[x]`）。次の1アクション: **6.3 接続文字列を db_common_dev へ変更**（`PrintAgent/appsettings.json` の `ConnectionStrings:CloudDb`）。

---

## print-platform 実装：タスク6.3 完了（親6 完了）＋環境確認

### 環境確認（ユーザー懸念に回答）
- **db_common_dev は実在**: MainWeb appsettings `CommonDb=...Database=db_common_dev...`。**SmtpAgent が既に `CloudDb=db_common_dev` で稼働中**（同一DB運用）。SQL Server=`OJIADM23120073\DEVELOPMENT`。
- **db_material_dev は material オリジナルとして不変**: 6.3 は PrintAgent の `CloudDb` 1行のみ変更。MainWeb `MaterialDb=db_material_dev` は不変、MaterialModule は従来どおり db_material_dev。db_material_dev の `t_order_reports` 等も保全（9.1 で削除せず）。
- PrintAgent の Development 設定に CloudDb 上書きなし → 変更対象は `appsettings.json` の1行のみ。

### 完了（6.3）
- `\\ojiadm23120073\Labs\WindowsService\PrintAgent\appsettings.json`: `CloudDb` の `Database=db_material_dev` → `db_common_dev`（SmtpAgent と同じ向き）。heartbeat 先も db_common_dev に。
- 親タスク **6 完了**（6.1 エンティティ／6.2 DbContext／6.3 接続先）。tasks 17/37 完了。

### 中間状態・次
- ⚠ `PrintJobWorker` が旧 `db.OrderReports` 参照のため **7.1 まで PrintAgent ビルド不可**（想定どおり）。
- 実行時前提: db_common_dev に `t_print_queue`／`m_print_agent_control`（DDL 1.1・`CommonModule/docs/sql`）をユーザー適用。
- **次の1アクション: 7.1 出力ソース選択（デュアルモード）と状態遷移を実装**（`PrintJobWorker`：待機取得条件を print_status=1 かつ(payload 非NULL or pdf_path 非NULL)、pdf_path 優先で直接印刷／無ければ payload 生成、1→2 picked_at、完了 3 printed_at、失敗 9 error_message、row_version 実効化。PDF生成・SumatraPDF印刷ロジックは不変）。これで PrintAgent が再びビルド可能に。
- 未コミット: PrintAgent（TPrintQueue.cs 追加・DbContext・appsettings）・Nonaka/.kiro（tasks 進捗・本メモ）。

---

## チェックポイント（本日終了・print-platform 実装中／design 要改訂＝PrintAgent は印刷専用）

### 本セッションの到達点
- print-platform 実装 **17/37 完了**（タスク1〜4＝Web側完了、6＝PrintAgent エンティティ/DbContext/接続先 完了）。
- 認可方針を確定（監視画面は MainWebホストで `[Authorize(Policy="DbPermissionCheck")]` 有効・権限登録＝m_content/r_content_auth は運用側）。作成した Common用 register SQL は不要のため削除済み。
- 重要訂正: **PrintAgent/SmtpAgent は編集可能範囲内**（パスは小文字 `ojiadm23120073` で扱う。大文字 `OJIADM` は範囲外誤判定）。
- 環境確認: db_common_dev 実在（SmtpAgent が既に使用）。db_material_dev は material オリジナルとして不変。t_print_queue は db_common_dev に一本化（旧 t_order_reports@db_material_dev は保全・未処理のみ 9.1 で移行）。

### コミット済み
- CommonModule `88473b3`（4.6 再出力＋4.8 ビュー）。
- Nonaka `9ae68e9`（tasks 進捗＋認可方針記録）。

### 未コミット（次回コミット対象）
- **PrintAgent**（`\\ojiadm23120073\Labs\WindowsService\PrintAgent`・別git）:
  - `?? Models/TPrintQueue.cs`（新規・6.1）
  - `M Data/PrintAgentDbContext.cs`（DbSet 差替え・6.2）
  - `M appsettings.json`（CloudDb→db_common_dev・6.3）
- **Nonaka/.kiro**: tasks.md（6.1/6.2/6.3/6 を [x]）＋本 session-memo。

### 🔴 最重要・次セッション最初の論点（design 改訂が必要）
ユーザー確定仕様: **PrintAgent は「受け取った PDF をサイレント印刷するだけ」。印刷イメージ（PDF）生成は送信側（投入側）の責務**。
→ 現行 design.md の **デュアルモード（D6＝pdf_path 無ければ payload から PrintAgent が生成）は誤り**。**PDF一本（pdf_path 必須・payload 生成経路なし）**へ是正する。

要改訂・是正項目:
1. **design.md**: D6 デュアルモード削除→「PrintAgent は pdf_path の PDF を印刷のみ」。Worker 待機取得条件を「print_status=1 かつ pdf_path 非NULL」。`PdfGeneratorService`/`Documents/*.cs` は PrintAgent から退役。
2. **requirements.md**: 出力ソース選択(R5.6)・Property 8(pdf_path 優先)・Property 2(payload or pdf_path) を PDF一本に修正。
3. **t_print_queue スキーマ**: pdf_path 必須化・print_payload 廃止（or 移行期残置＝要判断）。DDL `create_t_print_queue.sql` 追随。
4. **実装済み CommonModule の是正**: `PrintQueueService.EnqueueAsync` 検証を「payload or pdf_path」→ **pdf_path 必須**／`Common_PrintMonitor` 再出力条件を pdf_path ベースへ。
5. **tasks.md**: 7.1 を「pdf_path を印刷するのみ」に簡素化。7.2(Property 8) 見直し。
6. **PDF 生成の担い手（未決・要ユーザー回答）**: 資材3帳票（発注書兼納入依頼書/工場請求/入庫伝票＝現 PrintAgent `Documents/*.cs`）の生成を送信側（MaterialModule もしくは共通PDF生成サービス）へ移設。**dispatch-monitoring-consolidation / MaterialModule 所有**の可能性大（print-platform は「pdf_path で PDF を渡す契約」＋「PrintAgent は印刷専用」を定義）。

### 次セッションの最初の1アクション
- ユーザーに確認: (1) design を上記 To-Be に改訂して良いか (2) PDF 生成の担い手 (3) print_payload 完全廃止 or 移行期残置。
- 合意後: design.md 改訂 → requirements/tasks 追随 → 実装済み CommonModule 是正 → PrintAgent 7.1（印刷専用）を最小単位で。
- ⚠ 現状 PrintAgent は 6.2 で `db.OrderReports` 参照が壊れており **7.1 まで PrintAgent ビルド不可**（想定どおり）。
...
## print-platform design 改訂完了（PrintAgent 印刷専用・PDF一本）＋ユーザー3決定

### ユーザー確定（3点）
1. design を To-Be（PrintAgent 印刷専用・送信側PDF生成）へ改訂 = OK
2. PDF 生成の担い手 = **a) MaterialModule 側で生成して pdf_path を渡す**
3. `print_payload` = **完全廃止**

### design.md 改訂（単一正本・診断クリア）
...
- Property 8（出力ソース選択）廃止・Property 2 を pdf_path 必須に修正・Property 1/3 の payload 参照除去（Property 1〜7＝PBT、9＝統合）。
- Error Handling／カットオーバー（残ジョブは pdf_path 必須＝送信側がPDF用意 or 除外）／責務分界（PDF生成は MaterialModule/dispatch 所有）追随。
- 「実装同期の注記」を design 内に明記：実装済み CommonModule（entity print_payload・service 検証・DDL・監視再出力条件）は tasks で是正。

### 次アクション（最小単位・順に）
1. **requirements.md 追随更新**（R5.6 出力ソース選択・Property 2/8・print_payload・pdf_path 必須）。
2. **tasks.md 追随更新**（7.1 を pdf_path 印刷のみに簡素化／7.2 Property 8 削除／CommonModule 是正タスク追加：entity print_payload 削除・PrintQueueService pdf_path 必須・IPrintQueueService シグネチャ・DDL・監視再出力条件／9.1 移行SQL pdf_path 必須）。
3. **実装是正（CommonModule）**：TPrintQueue から print_payload 削除・PrintQueueService/IPrintQueueService・create_t_print_queue.sql・Common_PrintMonitor 再出力条件。
4. **PrintAgent 7.1**：印刷専用（pdf_path サイレント印刷）。TPrintQueue(PrintAgent) は print_payload 未実装なので確認。PdfGeneratorService/Documents 退役の扱い（削除 or 保留）は要判断。
- ⚠ PrintAgent は 6.2 で `db.OrderReports` 参照が壊れ 7.1 までビルド不可（想定どおり）。
- 未コミット: PrintAgent（6.1/6.2/6.3）・Nonaka/.kiro（design 改訂・tasks・本メモ）。

---

## print-platform requirements.md 追随更新 完了（印刷専用・pdf_path必須）

- Glossary: `PrintPayload` 廃止→`pdf_path`（必須・唯一の印刷ソース）。PrintAgent=印刷専用に。t_order_reports/PrintJobService 記述追随。host小文字化。
- R1: 列一覧に module/pdf_path/printer_name、print_payload 削除。AC5=pdf_path 必須(NOT NULL・唯一の印刷ソース)、新AC6=print_payload 列を持たない。printed_at/parity は後ろにシフト。
...
- R9.5: 再出力可否を pdf_path 基準へ。R11.2: 残ジョブ移行に pdf_path 付与。R13: AC3 印刷専用化所有・新AC6 PDF生成非所有。
- 診断クリア。全要件の再詳細化はしない（外科的修正のため）。

### 次アクション: tasks.md 追随更新（1つずつ）
...
- PrintAgent 12（7.1 相当）: PdfGeneratorService/Documents 退役の扱い（削除 or 保留）を明記。9.1 移行SQL pdf_path 必須。
- Property 一覧 1〜7＝PBT・9＝統合（8 廃止）を tasks に反映。
