...
> **【改訂 2026/07/09：ユーザー別設定＋添付ファイル】** 送信設定を **ログインユーザー別**に保存/読込できるよう拡張する（R9）。`m_send_config` に `owner_user_id`（NULL=default 行）を追加し、ユーザー行が無い場合は default 行を初期表示値とする。**投入側（発注承認 FAX 等の業務送信）は常に default 行を採用**し、ユーザー別設定は管理画面の単発テスト送信でのみ有効とする（業務送信に属人化を持ち込まない）。あわせて **単発テスト送信の添付ファイル**（固定パス1つ・空なら添付なし・default は空・読込可否は送信時に判定）を追加する（R10）。R1（列定義）・R2（サービス）・R3（画面）・R6（単発テスト送信）は本改訂で拡張される（各所に追随注記）。
...
- **owner_user_id（所有ユーザーID）**【2026/07/09 追加】: 送信設定行の所有者を表すログインユーザーID（SharedCore `IUserRepository` 由来）。**NULL = default 行**（全ユーザー共通・投入側および未設定ユーザーの初期表示に使用）。値あり = 当該ユーザー専用行。
- **default 行**【2026/07/09 追加】: `owner_user_id IS NULL`（かつ `is_active=1`）の行。ユーザー行が無いときの**初期表示値**であり、**業務送信（投入側）が常に採用**する共通設定。
- **ユーザー別設定**【2026/07/09 追加】: `owner_user_id = ログインユーザーID` の行。管理画面の表示/保存と、当該ユーザーの単発テスト送信で用いる。
- **添付ファイルパス（attachment_path）**【2026/07/09 追加】: 単発テスト送信に添付する固定ファイルのフルパス。空/NULL は添付なし。default は空。読込可否は送信時に判定する。
...
### Requirement 9: 送信設定のユーザー別保存/読込（default フォールバック）

**実装状態: 未実装（2026/07/09 追加・改訂）**

**User Story:** 送信基盤の利用者として、自分のログインユーザー単位で送信設定（送信元・テスト宛先・添付）を保存・読込したい。開発担当ごとにテスト条件が異なっても、共通の default を壊さずに各自の設定で疎通確認するため。

#### Acceptance Criteria

1. THE 送信設定マスタ SHALL 列 `owner_user_id`（NVARCHAR・NULL 許容）を保持する。`owner_user_id IS NULL` を **default 行**とし、値ありを当該ユーザー専用行とする。
2. THE 送信設定マスタ SHALL 同一 `owner_user_id`（NULL を含む）に対する有効行が高々1件となるよう運用する（ユーザー単位・default 単位で1行）。
3. WHEN 送信設定管理画面（`/Common/SendConfig`）を表示し、かつログインユーザーの行（`owner_user_id = ログインユーザーID`）が存在する、THE 送信設定管理画面 SHALL 当該ユーザー行を編集フォームに表示する。
4. IF ログインユーザーの行が存在しない、THEN THE 送信設定管理画面 SHALL **default 行（`owner_user_id IS NULL`）を初期表示値**として表示する（新規保存でユーザー行が作成される）。
5. WHEN ログインユーザーが保存を実行する、THE 送信設定管理画面 SHALL `owner_user_id = ログインユーザーID` の行を作成または更新する（default 行は書き換えない）。
6. THE 投入側（発注承認 FAX 等の業務送信）SHALL 送信元 From・テスト宛先として **default 行（`owner_user_id IS NULL`）** を採用する（ユーザー別設定は業務送信に影響しない）。
7. THE ログインユーザーID SHALL SharedCore の `IUserRepository`（認証ユーザー）由来とする。
8. THE 単発テスト送信 SHALL 実行ユーザーの設定（ユーザー行が無ければ default 行）に基づく送信元・テスト宛先・添付を用いる。

### Requirement 10: 単発テスト送信の添付ファイル（送信時に読込可否を判定）

**実装状態: 未実装（2026/07/09 追加・改訂）**

**User Story:** 送信基盤の利用者として、単発テスト送信に任意の添付ファイルを付けたい。実運用に近い形（添付あり）で送信経路を検証するため。

#### Acceptance Criteria

1. THE 送信設定マスタ SHALL 列 `attachment_path`（NVARCHAR(500)・NULL 許容）を保持し、単発テスト送信に添付する固定ファイルのフルパスを表す。
2. THE 送信設定管理画面 SHALL `attachment_path` を編集できる。
3. WHERE `attachment_path` が空/NULL、THE 単発テスト送信 SHALL 添付なしで投入する。
4. WHEN 単発テスト送信を実行し、かつ `attachment_path` が非空である、THE 単発テスト送信 SHALL **送信時に当該パスのファイルが読込可能か判定**し、読込可能な場合のみ当該ファイルを添付（`pdfPath` として付与）して投入する。
5. IF `attachment_path` が非空かつ当該パスのファイルが読込できない、THEN THE 単発テスト送信 SHALL 投入せず、読込できない旨のエラーメッセージを表示する。
6. THE default 行の `attachment_path` SHALL 既定で空とする。
