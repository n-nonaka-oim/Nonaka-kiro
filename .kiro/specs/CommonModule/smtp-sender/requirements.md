...
- **接続プロファイルキー(config_key)**: 接続プロファイルを識別する主キー。送信ジョブが使用する接続プロファイルを選択するために保持する(例: `mail`, `fax`, `test-fax`)。送信元モジュールは送信種別に応じて `mail`(メール直送) / `fax`(FAX送信) / `test-fax`(テストFAX・固定宛先) から選定する。`Material`・`test` は廃止する。
  - **【改訂 2026/07/08：取り下げ】** `test-fax`(固定宛先)方式は**取り下げ**。正となるテスト送信は **recipient 上書き方式**（config_key は本番と同じ `fax`/`mail` のまま、宛先のみを送信設定マスタ `m_send_config` のテスト宛先に差し替える）であり、その責務は spec `send-config-master`（R4/R5）と `dispatch-monitoring-consolidation` が所有する。したがって**運用する config_key は `mail`/`fax` の2モードを正**とし、`test-fax`（固定宛先）は**非推奨・未使用**（SmtpAgent 実装は残存するが投入側が選択しない・撤去は任意）。
...
- **送信モード(SendMode)**: 接続プロファイルの `fax_domain` の値の形から決まる、宛先解決の3モード。
  ...
  - **固定宛先モード(test-fax)**: `fax_domain` が `@` の前に局所部を持つ完全アドレス(例: `0064871033@faxmail.com`)。送信ジョブの宛先を無視し、`fax_domain` の値そのものを送信先(To)とする。**【改訂 2026/07/08：非推奨・未使用】** 本モードはテスト送信の旧方式であり取り下げ済み。現行のテスト送信は recipient 上書き方式（config_key は `fax`/`mail` のまま）に統一されたため、固定宛先モードは投入側から選択されない（SmtpAgent 実装は残存・撤去は任意）。運用上の送信モードは **メール直送 / FAX送信 の2モード**を正とする。
...
- **テスト送信指定(TestSendSelection)**: 送信元モジュールが**投入操作ごと**に指定する、テストFAX送信とするか否かの選択。**永続的・全体共有の状態(グローバルなトグル)としては保持しない**(多人数同時運用での取り違え・競合を避けるため、投入する各ジョブに対してのみ効果を持つ)。発注承認FAXでは承認画面(Approvals)のチェックボックスで承認操作ごとに指定する。
  - **【改訂 2026/07/08】** テスト指定時の解決方式を変更。旧方式（`config_key` に `test-fax` を指定し SmtpAgent が固定宛先へ送る）は取り下げ。**現行は recipient 上書き方式**：`config_key` は本番と同じ `fax`(FAX)／`mail`(メール)のまま、投入側が宛先を送信設定マスタ `m_send_config` の `test_fax_number`／`test_email` に差し替える。宛先解決値の管理・上書き処理は spec `send-config-master`／`dispatch-monitoring-consolidation` が所有する。
...
6. WHERE 接続プロファイルの FAXゲートウェイドメイン(`fax_domain`)が完全アドレス(`@` を含み局所部が空でない、例: `0064871033@faxmail.com`)である、THE 接続プロファイル SHALL 固定宛先(test-fax)モード用の接続として扱われる。**【改訂 2026/07/08：非推奨・未使用】** 固定宛先(test-fax)モードは取り下げ済み。運用する送信モードは メール直送 / FAX送信 の2モードを正とする（本AC の判定ロジック自体は SmtpAgent に残存するが投入側から選択されない）。
7. THE 接続プロファイルマスタ SHALL 接続プロファイルキー `Material` および `test` を保持しない(廃止)。運用する接続プロファイルキーは `mail`・`fax`・`test-fax` とする。**【改訂 2026/07/08】** 運用する config_key は `mail`・`fax` の2モードを正とする。`test-fax` 行は残置可だが投入側から選択されない（テスト送信は recipient 上書き方式・`send-config-master` 所有）。
...
> **【改訂 2026/07/08：固定宛先モードは取り下げ・非推奨】** 宛先解決は **メール直送 / FAX送信 の2モードを正**とする。固定宛先(test-fax)モード（AC8）はテスト送信の旧方式であり取り下げ済みで、投入側から選択されない（SmtpAgent 実装は残存・撤去は任意）。テスト送信は recipient 上書き方式（`config_key` は `fax`/`mail` のまま、宛先を `m_send_config` のテスト宛先へ差し替え）に統一され、その責務は `send-config-master`／`dispatch-monitoring-consolidation` が所有する。
...
> **【改訂 2026/07/08：本要件は取り下げ／方式変更】** 本 requirement の「`config_key=test-fax`（固定宛先モード）でテスト送信する」方式は**取り下げ**。現行の正となるテスト送信は **recipient 上書き方式**：`config_key` は本番と同じ `fax`(FAX)／`mail`(メール)のまま、投入側が宛先を送信設定マスタ `m_send_config` の `test_fax_number`／`test_email` に差し替える。テスト宛先値の管理・上書き・「ジョブ単位・非共有・競合回避」の担保は spec **`send-config-master`（R1・R4・R5）** および **`dispatch-monitoring-consolidation`** が所有する。以下 AC1〜AC8（`test-fax` 前提）は履歴として残置するが**無効**（正は上記2 spec を参照）。
...
- 自動リトライ機能
- テスト用の固定宛先そのものの値管理は接続プロファイルマスタ(`test-fax` の `fax_domain`)の責務とする。送信元モジュールは投入ジョブごとに `config_key`(`fax`/`test-fax`) を選定するのみで、テスト用の宛先値は保持しない。**【改訂 2026/07/08：取り下げ】** この方式は取り下げ済み。テスト宛先値は送信設定マスタ `m_send_config`（`test_fax_number`／`test_email`・`send-config-master` 所有）で管理し、投入側が recipient を上書きする（`config_key` は `fax`/`mail` のまま）。本 spec（smtp-sender）はテスト宛先値を保持しない。
