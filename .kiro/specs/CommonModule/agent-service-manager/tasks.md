...
- [x] 8. リモート対応拡張（ローカル/リモート・複数ターゲット）【2026/07/10 スコープ拡張・完了】
  ...
  - [x] 8.5 チェックポイント（ユーザー・リモート実機）: 別サーバの Common*Agent を起動/停止/登録/解除・到達不可/権限不足時に当該行のみエラーで他行継続（2026/07/10・サーバ側確認OK）
    - ドメイン構成。d86223 を対象サーバのローカル管理者に追加＋クライアントで管理者昇格実行で解決。到達/権限/配置の切り分けは docs のトラブルシュートに反映。
    - _Requirements: 8.2, 10.1, 10.2_

- [x] 9. 実行時ターゲット編集（入力UI・保存）【2026/07/10 追加・完了】
  ...
  - [x] 9.3 「設定に保存」ボタン（現一覧を `ConfigWriter` で appsettings へ永続化）（2026/07/10）
    ...
    - _Requirements: 11.4_
  - [x] 9.4 チェックポイント（ユーザー）: 追加→行表示/操作対象化・除外・保存→再起動後も反映（2026/07/10・ローカル/サーバ確認OK）
