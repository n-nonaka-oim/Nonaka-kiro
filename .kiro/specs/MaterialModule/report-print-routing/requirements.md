...
4. IF 承認対象に**出力区分が印刷を含む（1 または 3）** 発注があり、その**発注者（`t_orders.user_id`）×`order_approval` の割当プリンタが未設定**, THEN THE 改修 SHALL **承認を実行せずエラーで中止**し（案B・承認前ブロック）、状態変更・採番・投入を一切行わない。
5. WHEN 承認をブロックした, THE 改修 SHALL **プリンタ未設定である旨をユーザーに通知**する（該当発注が分かるメッセージ・「印刷設定」での登録を促す）。
