...
- **投入インターフェース契約 `IPrintQueueService.EnqueueAsync`**（CommonModule 所有・実装済み）:
  ...
  Task<int> EnqueueAsync(
      ...
      int outputType,       // 出力区分（印刷対象判定に用いる）
...
flowchart TD
    ...
    C --> D["IOrderPdfService.GenerateGroupOrderPdfAsync<br/>（発注書兼納入依頼書 PDF 生成・再利用）"]
    D --> E["印刷出力パスマスタから<br/>ベースパス取得（DBマスタ・R9）"]
    E --> F["PDF をベースパス配下に保存<br/>→ フルパス(pdf_path)確定"]
    F --> G["IPrintQueueService.EnqueueAsync<br/>module=material / reportType=order_approval /<br/>referenceCode=groupKey / outputType /<br/>pdfPath=保存パス / printerName=null / copies"]
...
- **処理手順（グループ単位。`DispatchEnqueueService` と同型）**:
  ...
  4. `IOrderPdfService.GenerateGroupOrderPdfAsync(groupKey)` で発注書兼納入依頼書 PDF（byte[]）を生成する（**再利用**）。
  ...
  6. `IPrintQueueService.EnqueueAsync("material", "order_approval", groupKey, outputType, fullPath, printerName: null, copies)` を呼び出す（`copies` は現行同様 1）。`outputType` はグループ代表（先頭）の `OutputType ?? 0`。
  7. 成功件数をカウントして返す。
...
| `outputType` | `output_type` | 代表発注の `OutputType ?? 0` |
...
*任意の* 承認済み発注集合に対して、`PrintJobService` は `OrderNo` 採番済み（非空）の発注のみを対象とし、発注番号グループ（`ExtractGroupKey` による先頭3セグメント）**1つにつき `IPrintQueueService.EnqueueAsync` をちょうど1回** 呼び出す。各呼び出しの引数は投入契約を満たす（`module == "material"`、`reportType == "order_approval"`、`referenceCode == ExtractGroupKey(該当OrderNo)` かつ非空、`pdfPath` が非空、`copies >= 1`）。未採番（`OrderNo` が空）の発注はいずれのグループにも寄与せず、投入を発生させない。
...
4. `PrintJobService` 改修（PDF生成＝`OrderPdfService` 再利用＋パス解決＋`IPrintQueueService` 投入・`t_order_reports` 非書込・FAX 非生成・`PrintPayload` 廃止）。`IPrintJobService` シグネチャは維持。
