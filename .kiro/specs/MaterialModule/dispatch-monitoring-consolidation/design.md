...
6. **FAX送信の config_key 選定と承認画面テスト送信（R10）**: FAXジョブ投入時の config_key を本番 `fax`／テスト `test-fax` から選定する（旧 `Material` 廃止）。テスト送信の要否は承認画面（Approvals）の「FAXテスト送信」チェックボックスで承認操作ごと（ジョブ単位）に指定し、`FaxDispatchOptions.TestSendEnabled`/`TestFaxNumber`（全体既定・宛先上書き）は廃止する。宛先解決・固定宛先モードの振る舞いは別 spec `smtp-sender`（R6/R8）が所有する。
   ...
   > **【改訂 2026/07/08：現行実装に追随／test-fax 選定は取り下げ】** 本項の「config_key を `fax`/`test-fax` から選定」は取り下げ。**現行（実装 `ab31934`）は次のとおり**：config_key は**常に `fax`**（`NormalConfigKey`）。テスト送信時は **recipient を送信設定マスタ `m_send_config.test_fax_number` に上書き**する（`test-fax` config_key・`FaxDispatchOptions.TestConfigKey` は**廃止**）。送信元 From は `m_send_config.from_address`（無ければ `FaxDispatchOptions.FromAddress` フォールバック）。テスト宛先値・From の管理は spec `send-config-master`（R1/R4/R5）が所有する。承認操作ごと（ジョブ単位・非共有）である点は不変。
...
> **【改訂 2026/07/08：現行実装＝recipient 上書き方式／以下本文の「test-fax 選定」は取り下げ・無効】**
> 現行（実装 `ab31934`）の正は次のとおり。以下の本文（`TestConfigKey`／`ResolveRecipientForSend` 廃止で宛先を上書きしない 等）は履歴として残置するが**無効**。
> - config_key は **常に `fax`**（`NormalConfigKey`）。`FaxDispatchOptions.TestConfigKey` は**廃止**。
> - テスト送信時は **`DispatchEnqueueService` が recipient を `m_send_config.test_fax_number` に上書き**する（`test_fax_number` 未設定はスキップ＋ログ）。本番は実FAX番号。
> - 送信元 From は `m_send_config.from_address`（無ければ `FaxDispatchOptions.FromAddress` フォールバック）。`ISendConfigService.GetActiveAsync` で取得。
> - テスト宛先値・From の管理・上書き契約は spec `send-config-master`（R1/R4/R5）が所有。`smtp-sender` の固定宛先(test-fax)モードには依存しない（2モード＝`fax`/`mail`）。
> - `t_order_dispatch_log` には `IsTestSend`＝testSend、`ConfigKey`＝`fax`（選定値）、recipient＝上書き後の宛先を記録。
...
> **【改訂 2026/07/08：現行実装で本 Property は反転／再定義】** 現行（実装 `ab31934`）は recipient 上書き方式のため、本 Property の内容は次に**読み替える**（下記の旧文＝「configKey が test-fax/fax・宛先は上書きしない」は無効）：
> **Property 4'（現行）**: 任意の承認済み発注集合（FAX対象 `OutputType ∈ {2,3}` を含む）とテスト送信フラグ `testSend` に対し、`EnqueueOrderApprovalFaxAsync(orders, testSend, ...)` が投入するとき、`ISmtpQueueService.EnqueueAsync` の `configKey` は **`testSend` に関わらず常に `fax`**（`NormalConfigKey`）である。かつ recipient は、`testSend == true` のとき **`m_send_config.test_fax_number`（マスタのテスト宛先）に上書き**され（未設定なら投入スキップ）、`testSend == false` のとき解決済みの実FAX番号となる。From は `m_send_config.from_address`（無ければフォールバック）。**Validates: Requirements 10.1, 10.3, 10.4, 10.6（現行の recipient 上書き方式・`send-config-master` R4/R5 と整合）**
...
- **旧監視画面の廃止・導線解除（R3/R5・タスク5.1/5.2/6.1）**: 投入先切替（③）後、旧 Material 監視画面（`Material_SmtpMonitor`／`Material_PrintMonitor`）は役割を終えるため**ページ実体を削除**し（監視は `/Common/SmtpMonitor`・`/Common/PrintMonitor` に集約済み）、導線（`dbAuthTest.m_content`／`r_content_auth` の Material 側 `SmtpMonitor/Index`・`PrintMonitor/Index`）を解除する。解除 SQL は `MaterialModule/docs/sql/unregister_material_monitor_content.sql`（`register_smtp_monitor_content.sql` と対・実行はユーザー）。エンティティ `TOrderReport`・`MaterialDbContext` の DbSet は保全（削除しない＝移行・保全期間依存。参照コードのみ除去）。ページ削除・導線解除は投入先切替が安定稼働してから実施する（可逆性のため、旧画面の復元は git／SQL 再登録で可能）。
